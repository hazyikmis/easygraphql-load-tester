...
exports.getField = function (field, schema, maxDeepLevel, deepLevel) {
    ...
    nestedType.fields.forEach(function (field) {
        if (schema[field.type]) {
            var nestedField = exports.getField(field, schema, maxDeepLevel, deepLevel);
            if (nestedField) {
...
exports.createQueryArguments = function (args, userArgs, queryName) {
    ...
    var validateArg = function (userArg, arg) {
        if (typeof userArg === 'undefined' && arg.noNull) {
            throw new Error("Error in " + queryName + " - All required query arguments must be defined - missing " + arg.name);
        }
    };
    args.forEach(function (arg) {
        ...
        var variableDefinition = arg.isArray
            ? "$" + arg.name + ": [" + arg.type + (arg.noNullArrayValues ? '!' : '') + "]" + (arg.noNull ? '!' : '')
...
exports.createUnionQuery = function (nestedType, schema, queryType, maxDeepLevel) {
    var fields = [];
    nestedType.fields.forEach(function (field) {
        var createdField = exports.getField(field, schema, maxDeepLevel, 2);
        if (createdField) {
...
exports.createQuery = function (_a) {
    var fields = _a.fields, queryHeader = _a.queryHeader, isMutation = _a.isMutation, name = _a.name, operationName = _a.operationName, variables = _a.variables;
    var selectedFields = '';
    if (fields.length > 0) {
    ...
    if (Array.isArray(variables)) {
        return variables.map(function (val) { return ({
            name: name,
            query: newQuery,
